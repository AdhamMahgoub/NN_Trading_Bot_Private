...
def compute_engineered_features(
    ...
    NaN dropped. If keep_row_index=True, also returns __row_index__ for alignment
    with caller-owned columns (e.g., labels). Does NOT compute labels.
    """
    df = df.copy()
    if keep_row_index:
    ...
    if keep_row_index:
        out = pd.concat([df[["Date", "__row_index__"]], Xdf], axis=1)
    else:
...
def featurize_one_csv(
    ...
    # Shared feature pipeline (keeps feature math/order identical between training and inference)
    features = compute_engineered_features(df, window=window, keep_row_index=True)
    labels = pd.DataFrame(
        {
